...
let partitionWorker: Worker | null = null;

if (connection) {
  partitionWorker = new Worker(
    "partition",
    async (_job: Job) => {
    ...
    LIKE "stock_movements" INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES
        ...
        try {
          ...
          log.error({ err: error, tableName }, "Failed to create partition");
          throw error;
        }
      }

      log.info({ partitions: partitionsCreated }, "Partition creation complete");
  ...
  logger.info("Redis not configured — partition worker disabled");
}
